...
import { HeartIcon, ShoppingCartIcon, ClockIcon, MapPinIcon} from 'react-native-heroicons/solid';
    ...
    <View className="bg-white flex-1">
        ...
        <View style={{borderTopLeftRadius: 40, borderTopRightRadius: 40}} className="px-5 flex flex-1 justify-between bg-white pt-8 -mt-14">
            <ScrollView showsVerticalScrollIndicator={false} className="space-y-5">
                ...
                <View className="flex-row justify-between mx-1">
                    <View className="flex-row space-x-2 items-start">
                        <ShoppingCartIcon size={wp(7)} color="#996dbc" />
                        <View className="flex space-y-2">
                            <Text style={{fontSize: wp(4.5)}} className="font-bold text-neutral-700">{item.purchase}</Text>
                            <Text className="text-neutral-600 tracking-wide">Purchase</Text>
                        </View>
                    </View>
                    <View className="flex-row space-x-2 items-start">
                        <MapPinIcon size={wp(7)} color="#f87171" />
                        <View className="flex space-y-2">
                            <Text style={{fontSize: wp(4.5)}} className="font-bold text-neutral-700">{item.trackedIn}</Text>
                            <Text className="text-neutral-600 tracking-wide">Tracked In</Text>
                        </View>
                    </View>
                    <View className="flex-row space-x-2 items-start">
                        <ClockIcon size={wp(7)} color="skyblue" />
                        <View className="flex space-y-2">
                            <Text style={{fontSize: wp(4.5)}} className="font-bold text-neutral-700">{item.claimIn}</Text>
                            <Text className="text-neutral-600 tracking-wide">Claim In</Text>
                        </View>
                    </View>
                </View>
            </ScrollView>
            <TouchableOpacity style={{backgroundColor: theme.bg(0.8), height: wp(15), width: wp(50)}} className="mb-6 mx-auto flex justify-center items-center rounded-full"
              onPress={() => Linking.openURL(item.aoURL)}
            >
                <Text className="text-white font-bold" style={{fontSize: wp(5.5)}}>Shop now!</Text>
            </TouchableOpacity>
        </View>
    </View>
  )
